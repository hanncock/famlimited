      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              _buildHeader(),
              (_images.isNotEmpty && _images[0].isNotEmpty) ? _buildMainImage() : null,
              _buildThumbnails(),
...
class _DispGalState extends State<DispGal> {
  ...
  void _previousImage() {
    setState(() {
      _currentImageIndex =
      _currentImageIndex > 0 ? _currentImageIndex - 1 : 0;
    });
  }

  void _nextImage() {
    setState(() {
      _currentImageIndex = _currentImageIndex < _images.length - 1
  ...
  Widget _buildMainImage() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        image: DecorationImage(
          image: CachedNetworkImageProvider("${auth.imgurl}/$currentImage"),
          fit: BoxFit.contain,
        ),
      ),
      child: Center(
      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              _buildHeader(),
              if (_images.isNotEmpty && _images[0].toString().isNotEmpty)
                _buildMainImage(),
              _buildThumbnails(),
              if (widget.data['videolink'] != null &&
                  widget.data['videolink'].toString().isNotEmpty)
                _buildVideoSection(),
            ],
          ),
        ),
